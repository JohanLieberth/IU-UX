...
function generarDocumentoMinuta(idMinuta) {
  try {
    ...
    const metaTableData = [
      ['Folio Minuta:', minuta.folio || 'N/A'],
      ['Proyecto:', nombreProyecto],
      ['Etapa del Proyecto:', minuta.etapa || 'N/A'],
      ['Dependencia:', minuta.dependencia || 'N/A'],
      ['Subdirección:', minuta.subdireccion || 'N/A'],
    ...
    const acuerdosTableData = [
      ['No.', 'Tipo', 'Descripción del Acuerdo', 'Prioridad', 'Solicitante(s)', 'Responsable(s)', 'Tracking / Ac. Ant.', 'Fecha Límite']
    ];

    if (acuerdos && acuerdos.length > 0) {
      acuerdos.forEach((item, idx) => {
        const trackingText = (item.tracking || '') + (item.num_acuerdo_anterior ? (' (' + item.num_acuerdo_anterior + ')') : '');
    ...
    formatTableStandard(acuerdosTable, [25, 55, 150, 50, 60, 60, 50, 50]);
